...
class JumpingSpiderEnvironmentBaseConfiguration(MuJoCoEnvironmentConfiguration):
    def __init__(
            self,
...
class JumpingSpiderEnvironmentBase:
    def __init__(self) -> None:
        self._segment_joints_qpos_adrs = None
        self._segment_joints_qvel_adrs = None
        self._leg_tip_capsule_geom_ids = None

    def _cache_references(self, mj_model: mujoco.MjModel) -> None:
        self._get_leg_tip_capsule_geom_ids(mj_model=mj_model)
        self._get_joints_qpos_adrs(mj_model=mj_model)
        self._get_joints_qvel_adrs(mj_model=mj_model)

    def _get_joints_qpos_adrs(self, mj_model: mujoco.MjModel) -> jnp.ndarray:
        if self._segment_joints_qpos_adrs is None:
            self._segment_joints_qpos_adrs = jnp.array(
                [
                    ...
                    if "freejoint" not in mj_model.joint(joint_id).name
                ]
            )
        return self._segment_joints_qpos_adrs

    def _get_joints_qvel_adrs(self, mj_model: mujoco.MjModel) -> jnp.ndarray:
    ...
    def _get_leg_tip_capsule_geom_ids(self, mj_model: mujoco.MjModel) -> jnp.ndarray:
        if self._leg_tip_capsule_geom_ids is None:
            self._leg_tip_capsule_geom_ids = jnp.array(
                [
                    geom_id
                    for geom_id in range(mj_model.ngeom)
                    if "_tarsus_" in mj_model.geom(geom_id).name
                       and "capsule" in mj_model.geom(geom_id).name
                ]
            )
        return self._leg_tip_capsule_geom_ids
    ...
    def _color_segment_capsule_contacts(
        ...
        for i, mj_model in enumerate(mj_models):
            ...
            for capsule_geom_id, contact in zip(
                    self._get_leg_tip_capsule_geom_ids(mj_model=mj_model), contacts
            ):
                if contact:
